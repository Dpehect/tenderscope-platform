...
app.MapGet("/health", () => Results.Ok(new { status = "healthy", service = "tenderscope-api", version = "1.5.0", utc = DateTimeOffset.UtcNow }));
...
app.MapGet("/api/stats", async (ITenderRepository tenders, ITenderSourceRepository sources, CancellationToken ct) =>
{
    var sourceList = await sources.ListAsync(ct);
    return Results.Ok(new { totalTenders = await tenders.CountAsync(ct), totalSources = sourceList.Count, healthySources = sourceList.Count(x => x.Health == SourceHealth.Healthy), generatedAt = DateTimeOffset.UtcNow });
});
...
var admin = app.MapGroup("/api/admin").AddEndpointFilter(async (context, next) =>
{
    ...
    if (string.IsNullOrWhiteSpace(configuredKey)) return Results.Unauthorized();
    var expected = System.Text.Encoding.UTF8.GetBytes(configuredKey);
    var actual = System.Text.Encoding.UTF8.GetBytes(suppliedKey.PadRight(configuredKey.Length)[..configuredKey.Length]);
    return CryptographicOperations.FixedTimeEquals(expected, actual) ? await next(context) : Results.Unauthorized();
});
...
admin.MapPost("/sources/{id:guid}/enabled", async (Guid id, bool enabled, TenderScopeDbContext db, HttpContext http, CancellationToken ct) =>
{
    var source = await db.Sources.FindAsync([id], ct);
    if (source is null) return Results.NotFound();
    source.SetEnabled(enabled);
    db.AuditLogs.Add(new AuditLog { Action = enabled ? "source.enabled" : "source.disabled", Resource = $"source:{id}", ActorKey = "admin", IpAddress = http.Connection.RemoteIpAddress?.ToString() });
