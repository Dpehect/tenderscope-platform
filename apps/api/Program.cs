...
app.Use(async (context, next) => { context.Response.Headers.XContentTypeOptions = "nosniff"; context.Response.Headers.XFrameOptions = "DENY"; context.Response.Headers.Append("Referrer-Policy", "strict-origin-when-cross-origin"); context.Response.Headers.ContentSecurityPolicy = "default-src 'none'; frame-ancestors 'none'; base-uri 'none'"; context.Response.Headers.Append("Permissions-Policy", "camera=(), microphone=(), geolocation=()"); await next(); });
app.UseRateLimiter(); app.UseCors(); app.UseAuthentication(); app.UseAuthorization();
app.MapOpenApi();
app.MapHealthChecks("/health/live", new HealthCheckOptions { Predicate = _ => false });
app.MapHealthChecks("/health/ready");
app.MapGet("/health", () => Results.Ok(new { status = "healthy", service = "tenderscope-api", version = "2.0.0", utc = DateTimeOffset.UtcNow }));
app.MapTenderScopeAuth(); app.MapOrganizationManagement(); app.MapTenantWorkspace(); app.MapWatchlists(); app.MapNotifications(); app.MapOrganizationAnalytics(); app.MapIntelligence(); app.MapAdminOperations();
...
var admin = app.MapGroup("/api/admin").AddEndpointFilter(async (context, next) => { var configuredKey = builder.Configuration["Admin:ApiKey"]; var suppliedKey = context.HttpContext.Request.Headers["X-Admin-Key"].ToString(); if (string.IsNullOrWhiteSpace(configuredKey)) return Results.Unauthorized(); var expected = System.Text.Encoding.UTF8.GetBytes(configuredKey); var actual = System.Text.Encoding.UTF8.GetBytes(suppliedKey.PadRight(configuredKey.Length)[..configuredKey.Length]); return CryptographicOperations.FixedTimeEquals(expected, actual) ? await next(context) : Results.Unauthorized(); });
...
await using (var scope = app.Services.CreateAsyncScope())
{
    ...
    await db.EnsureIdentitySchemaAsync(); await db.EnsureWorkspaceTenantSchemaAsync(); await db.EnsureNotificationSchemaAsync();
    ...
    var sources = scope.ServiceProvider.GetRequiredService<ITenderSourceRepository>(); foreach (var seed in new[] { demo, ted }) if (await sources.FindByKeyAsync(seed.Key, CancellationToken.None) is null) await sources.AddAsync(seed, CancellationToken.None); await sources.SaveChangesAsync(CancellationToken.None);
}
app.Run();
