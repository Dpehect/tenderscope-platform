...
export async function login(input: LoginInput) {
  const session = await publicRequest<AuthSession>('/api/auth/login', { method: 'POST', body: JSON.stringify(input) });
  storeSession(session);
  return session;
}

export async function register(input: RegisterInput) {
  const session = await publicRequest<AuthSession>('/api/auth/register', { method: 'POST', body: JSON.stringify(input) });
  storeSession(session);
  return session;
}
...
export async function refreshSession(): Promise<AuthSession | null> {
  ...
  refreshPromise = (async () => {
    try {
      const session = await publicRequest<AuthSession>('/api/auth/refresh', { method: 'POST' });
      storeSession(session);
      return session;
    } catch {
      storeSession(null);
...
export async function logout() {
  try { await publicRequest<void>('/api/auth/logout', { method: 'POST' }); }
