...
export function AuthForm({ mode }: { mode: Mode }) {
  ...
  return <section className="authPanel">
    <div className="authIntro">
      <span className="kicker">SECURE ORGANIZATION ACCESS</span>
      <h1>{mode === 'login' ? 'Return to your procurement workspace.' : 'Create your TenderScope workspace.'}</h1>
      <p>{mode === 'login' ? 'Continue your organization pipeline, assignments and audit trail.' : 'Create an isolated organization workspace with owner-level access.'}</p>
    </div>
    <form className="authForm" onSubmit={submit}>
      {mode === 'register' && <>
        <label>Full name<input name="displayName" autoComplete="name" minLength={2} maxLength={160} required /></label>
        <label>Organization name<input name="organizationName" autoComplete="organization" maxLength={180} placeholder="Optional" /></label>
      </>}
