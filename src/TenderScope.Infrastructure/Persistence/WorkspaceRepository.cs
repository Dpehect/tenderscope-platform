...
public sealed class WorkspaceRepository(TenderScopeDbContext db) : IWorkspaceRepository
{
    public async Task<IReadOnlyList<WorkspaceItem>> ListItemsAsync(Guid organizationId, CancellationToken cancellationToken) =>
        await db.WorkspaceItems.AsNoTracking().Where(x => x.OrganizationId == organizationId).OrderBy(x => x.Stage).ThenBy(x => x.Position).ThenByDescending(x => x.UpdatedAt).ToListAsync(cancellationToken);
    ...
    public async Task<WorkspaceItem> SaveItemAsync(Guid organizationId, Guid userId, Guid tenderId, OpportunityStage stage, string? notes, CancellationToken cancellationToken)
    {
        ...
        if (item is null)
        {
            ...
            var lastPosition = await db.WorkspaceItems.Where(x => x.OrganizationId == organizationId && x.Stage == stage).Select(x => (decimal?)x.Position).MaxAsync(cancellationToken) ?? 0;
    ...
    {
        var entity = SavedSearch.Create(organizationId, userId, search.Name, search.Query, search.Country, search.Category, search.NotificationsEnabled);
