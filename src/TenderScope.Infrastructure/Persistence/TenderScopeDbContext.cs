...
public sealed class TenderScopeDbContext(DbContextOptions<TenderScopeDbContext> options) : DbContext(options)
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        search.Property(x => x.Name).HasMaxLength(180); search.Property(x => x.Query).HasMaxLength(500); search.Property(x => x.Country).HasMaxLength(3); search.Property(x => x.Category).HasMaxLength(120);
        search.HasOne<Organization>().WithMany().HasForeignKey(x => x.OrganizationId).OnDelete(DeleteBehavior.Cascade);
        search.HasOne<AppUser>().WithMany().HasForeignKey(x => x.CreatedByUserId).OnDelete(DeleteBehavior.Restrict);
